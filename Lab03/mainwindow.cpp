    ...
    void MainWindow::on_shine_Button_clicked()
    {
        ...
        int slider_val = ui->horizontalSlider->value();
        int slider_min = ui->horizontalSlider->minimum();
        int slider_max = ui->horizontalSlider->maximum();

        // Normalize value (0.0 to 1.0)
        double normalized = (slider_max > slider_min) ? (double)(slider_val - slider_min) / (slider_max - slider_min) : 0.0;

        const int MAX_DELAY_MS = 1000; // Slowest speed (at slider min)
        const int MIN_DELAY_MS = 50;  // Fastest speed (at slider max)

        // Linearly map the slider: min_slider -> MAX_DELAY, max_slider -> MIN_DELAY
        int delay_ms = MAX_DELAY_MS - (normalized * (MAX_DELAY_MS - MIN_DELAY_MS));
        int delay_us = delay_ms * 1000; // Convert to microseconds for usleep


        QFutureWatcher<void> *watcher = new QFutureWatcher<void>(this);
        connect(watcher, &QFutureWatcher<void>::finished, this, &MainWindow::onTaskFinished);

        QFuture<void> future = QtConcurrent::run(this, &MainWindow::perform_shine, count, delay_us);
    ...
    void MainWindow::perform_shine(int count, int delay_us) // Added delay_us
    {
        for (int i = 0; i < count * 2; i++) {
            if (i % 2 == 0) {
                ...
                //usleep(delay_us); // Use parameter instead of 300000
