	...
	BEGIN
		IF(rising_edge(Clock))THEN
			...
			IF(opcode="1110") THEN -- Custom Reset
				...
				opcode4 <= "1111";
				divisor_process <= '0';
				div_is_running <= '0'; -- Reset divider control logic
				
			ELSE
				IF(divisor_process = '1')THEN
					-- Pipeline is stalled for division. Control the external divider.
					LEDR <= div_quotient_out;
					LEDG <= div_state_out; -- Display divider's internal state for debugging
					
					IF (div_is_running = '0') THEN
					    -- First cycle of the stall. Start the division.
					    div_start <= '1'; -- Pulse 'clear' on the divider to start it.
					    div_is_running <= '1';
					    divisor_led <= '1';
					    
					    -- Find which pipeline stage is in Execution and provide its operands to the divider
                        IF(register1 = Execution)THEN
                            div_dividend_in <= RS1;
                            div_divisor_in <= RT1;
                        ELSIF(register2 = Execution)THEN
                            div_dividend_in <= RS2;
                            div_divisor_in <= RT2;
                        ELSIF(register3 = Execution)THEN
                            div_dividend_in <= RS3;
                            div_divisor_in <= RT3;
                        ELSIF(register4 = Execution)THEN
                            div_dividend_in <= RS4;
                            div_divisor_in <= RT4;
                        END IF;
					ELSE
					    -- Division is already running. Wait for it to finish.
					    -- div_start is already '0' by default.
					    IF (div_done = '1') THEN
					        -- Division has finished this cycle.
					        divisor_process <= '0'; -- Un-stall the pipeline for the next cycle.
					        div_is_running <= '0';  -- Reset for a future division.
					        divisor_led <= '0';
					        
					        -- Write the result into the destination register of the instruction in the Execution stage.
				...
				ELSE -- Normal Pipeline Operation
					div_is_running <= '0'; -- Ensure divider control is reset when not stalling.
					LEDR <= R0; -- Default LEDR output during normal operation
					LEDG <= "000";
					divisor_led <= '0';
				
					-- The following four CASE statements implement a rotating 4-stage pipeline.
					...
					CASE register1 IS
						...
						WHEN ID=>
							IF(opcode4 /= "1111" AND opcode1 /= "1111" AND (codeRS1 = codeRS4 OR codeRT1 = codeRS4))THEN
								Hazard <= '1';
								IF(codeRS1 = codeRS4 AND codeRT1 = codeRS4)THEN -- Both operands need forwarding
									RS1 <= RS4; RT1 <= RS4;
								ELSIF(codeRS1 = codeRS4)THEN -- Only RS needs forwarding
									RS1 <= RS4;
									CASE codeRT1 IS WHEN "00" => RT1 <= R0; WHEN "01" => RT1 <= R1; WHEN "10" => RT1 <= R2; WHEN "11" => RT1 <= R3; END CASE;
								ELSIF(codeRT1 = codeRS4)THEN -- Only RT needs forwarding
									RT1 <= RS4;
						...
						WHEN Execution=>
							CASE opcode1 IS
								...
								WHEN "1000" => null; -- Result is written by the divider control logic when done
								WHEN OTHERS => null;
							END CASE;
							IF(opcode1 = "1111")THEN Execution_led <= '0'; ELSE Execution_led <= '1'; END IF;
							register1<=WriteBack;
						WHEN WriteBack=>
							IF(opcode1 /= "1111")THEN -- Write result to the destination register
								CASE codeRS1 IS 
									WHEN "00" => R0 <= RS1; WHEN "01" => R1 <= RS1;
									WHEN "10" => R2 <= RS1; WHEN "11" => R3 <= RS1;
								END CASE;
							END IF;
