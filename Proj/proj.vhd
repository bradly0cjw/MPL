-- =================================================================================
-- File: cpu_pipeline_timing_aware.vhd
...
--   to control muxes in the EXE stage. This breaks a long combinational path in the original design,
--   ensuring the design can meet timing at higher clock frequencies.
-- - Added more comments to clarify the data flow and hazard handling.
-- =================================================================================
...
architecture behavioral of cpu_pipeline is
    ...
    -- Signal mapping for inputs
    signal data_in        : std_logic_vector(7 downto 0) := sw(7 downto 0);
    signal instruction_in : std_logic_vector(7 downto 0) := sw(15 downto 8);

    -- Constants
    constant OPCODE_WIDTH   : integer := 4;
    constant REG_ADDR_WIDTH : integer := 2;
    constant REG_COUNT      : integer := 2**REG_ADDR_WIDTH;
    ...
    -- Forwarding control: 00=No Forward, 01=Forward from EXE/WB, 10=Forward from WB/MEM (not used here)
    signal fwd_a_sel, fwd_b_sel : std_logic_vector(1 downto 0);

    -- Pipeline Registers and their inputs
    signal if_id_instr, if_id_instr_next : std_logic_vector(7 downto 0);

    signal id_exe_opcode, id_exe_opcode_next : std_logic_vector(OPCODE_WIDTH-1 downto 0);
    signal id_exe_rs_val, id_exe_rs_val_next : std_logic_vector(DATA_WIDTH-1 downto 0);
    signal id_exe_rt_val, id_exe_rt_val_next : std_logic_vector(DATA_WIDTH-1 downto 0);
    signal id_exe_write_en, id_exe_write_en_next : std_logic;
    signal id_exe_dest_addr, id_exe_dest_addr_next : std_logic_vector(REG_ADDR_WIDTH-1 downto 0);
...
begin
    ...
    begin
        if reset_n = '0' then
            ...
            end if; -- If stalled, IF/ID register holds its value
            ...
            if stall = '1' then
                ...
                id_exe_opcode      <= id_exe_opcode_next;
                id_exe_rs_val      <= id_exe_rs_val_next;
                id_exe_rt_val      <= id_exe_rt_val_next;
                id_exe_write_en    <= id_exe_write_en_next;
                id_exe_dest_addr   <= id_exe_dest_addr_next;
            end if;
    ...
    -- This process decodes, checks for hazards, and determines forwarding.
    ---------------------------------------------------------------------------
    ID_and_Hazard_Unit_proc: process(if_id_instr, id_exe_opcode, id_exe_dest_addr, exe_wb_write_en, exe_wb_dest_addr, register_file, instruction_in, div_state)
        variable v_id_opcode, v_id_rs_addr, v_id_rt_addr : std_logic_vector(3 downto 0);
    begin
        -- IF Stage Logic
        if_id_instr_next <= instruction_in;

        -- ID Stage: Instruction Decode
        v_id_opcode  := if_id_instr(7 downto 4);
        v_id_rs_addr := "00" & if_id_instr(3 downto 2);
        v_id_rt_addr := "00" & if_id_instr(1 downto 0);

        -- ID Stage: Register File Read (before forwarding logic)
        id_exe_rs_val_next <= register_file(to_integer(unsigned(v_id_rs_addr)));
        id_exe_rt_val_next <= register_file(to_integer(unsigned(v_id_rt_addr)));
        
        -- ID Stage: Control Signal Generation
        id_exe_opcode_next <= v_id_opcode;
        id_exe_dest_addr_next <= v_id_rs_addr(1 downto 0);
        case v_id_opcode is
            when OP_LOAD | OP_MOVE | OP_ADD | OP_SUB | OP_AND | OP_OR | OP_NOR | OP_SLT | OP_DIV =>
                id_exe_write_en_next <= '1';
            when others =>
                id_exe_write_en_next <= '0';
        end case;
        ...
        if (id_exe_opcode = OP_LOAD and id_exe_write_en = '1') and
           ((id_exe_dest_addr = v_id_rs_addr(1 downto 0)) or
            (id_exe_dest_addr = v_id_rt_addr(1 downto 0) and v_id_opcode /= OP_LOAD and v_id_opcode /= OP_MOVE)) then
            stall <= '1';
        end if;
        -- 2. Multi-cycle DIV hazard: Stall if a DIV is in progress in the EXE stage.
        if id_exe_opcode = OP_DIV and div_state /= S_IDLE and div_state /= S_DONE then
            stall <= '1';
        end if;

        -- Hazard Unit: Forwarding Control
        fwd_a_sel <= "00"; -- Default: no forward for operand A (Rs)
        if (exe_wb_write_en = '1') and (exe_wb_dest_addr = id_exe_dest_addr_next) then
             fwd_a_sel <= "01"; -- Forward from WB stage
        end if;

        fwd_b_sel <= "00"; -- Default: no forward for operand B (Rt)
        if (exe_wb_write_en = '1') and (exe_wb_dest_addr = v_id_rt_addr(1 downto 0)) then
             fwd_b_sel <= "01"; -- Forward from WB stage
        end if;
    end process;
    ...
    ---------------------------------------------------------------------------
    EXE_stage_proc: process(id_exe_opcode, id_exe_rs_val, id_exe_rt_val, exe_wb_result, fwd_a_sel, fwd_b_sel, data_in, div_state, div_Q)
    begin
        -- EXE Stage: Forwarding Multiplexers
        case fwd_a_sel is
            when "01"   => alu_operand_a <= exe_wb_result; -- Forward from WB
            when others => alu_operand_a <= id_exe_rs_val; -- Use value from ID
        end case;
        case fwd_b_sel is
            when "01"   => alu_operand_b <= exe_wb_result; -- Forward from WB
            when others => alu_operand_b <= id_exe_rt_val; -- Use value from ID
        end case;
    ...
    ledg(1) <= '1' when id_exe_opcode_next /= OP_NOP and stall = '0' else '0';
